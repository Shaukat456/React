...
👉 **Hooks solved this** by allowing _state, side effects, and context_ directly in _functional components_.
...
Think of them as **bridge functions** between your component and React’s internal _engine_ (called the **Fiber Reconciler**).
...
But stateful logic breaks purity — Hooks reintroduce _controlled impurity_, safely.
...
Hooks give the illusion of _persistence_ across renders — though your function is re-run each time.
...
1. **Call Hooks only at the top level**
   ...
   - ❌ Wrong:
     ...
     ```
     React relies on _call order consistency_ to match state across renders.
...
Each component’s hooks are **tracked in order** by React’s _Fiber tree_.
...
That’s why call order must _never change_ — or React’s internal “hook memory slots” desynchronize.
...
- You don’t “control lifecycle” anymore — you _declare side effects_.
- You don’t “inherit logic” — you _compose logic_.
- You don’t “mutate UI” — you _describe state changes_, and React handles the rest.
...
> Hooks are not magic — they’re **clever functions** that synchronize your component’s _render cycles_ with React’s _internal memory_, allowing functional components to behave like mini, reactive, stateful systems — without classes.
...
> Hooks are like the _organs of a living component_:
> `useState` is memory,
